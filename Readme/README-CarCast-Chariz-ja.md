...
**CarCast** は、**CarPlay 非対応**のアプリを **iOS 26** の **CarPlay ダッシュボード**に追加できる Tweak です。

**設定 → CarCast** から Safari や YouTube、設定アプリなどを選び、CarPlay に接続すれば、純正アプリと同じようにホーム画面から起動できます。

> **動作確認は iOS 26 です。** パッケージの最低依存は **iOS 17.0** です。**iOS 16 以前は非対応**です。**iOS 17〜18 は未検証**で、動く可能性はありますが保証はありません。動かなくても **購入者の責任**であり、**返金対象外**です。購入前に **設定 → 一般 → 情報** で OS バージョンを確認してください。
...
**Q. iOS 17 / iOS 18 で使えますか？**
A. **未確認です。** 作者が動作確認したのは **iOS 26 のみ**です。パッケージは **iOS 17 以降**に入るので、動く可能性はあります。**保証はありません。** 未検証の OS で購入して動かなくても、それは **購入者の責任**であり、**返金対象外**です。

**Q. iOS 16 以前で使えますか？**
A. **使えません。** iOS 16 以下は **非対応**です。パッケージも iOS 17 未満には入りません。それらのバージョンでの購入は **返金対象外**です。

**Q. CarBridge と CarCast を一緒に使えますか？**
A. **いいえ。** 競合します。CarCast は **iOS 26** 向けの CarBridge 代替です。どちらか一方だけをインストールしてください。
...
**Q. iOS 26 以外で買ったら動きませんでした。返金できますか？**
A. **できません。** **iOS 16 以前**は非対応です。**iOS 17 / iOS 18** は未検証です。どちらの場合も、お使いの OS で動かない購入は、インストールできても **返金ポリシーの対象外**です。
...
| **動作確認**         | **iOS 26**                                  |
| **最低依存（パッケージ）** | iOS 17.0                              |
| **未検証**           | iOS 17、iOS 18（動く可能性あり・保証なし）  |
| **非対応**           | iOS 16 以前                                 |
...
- **購入前に必ず iOS バージョンを確認してください。** 作者が動作確認したのは **iOS 26 のみ**です。
- **iOS 16 以前**は **非対応**です。**iOS 17 / iOS 18** は **未検証**です。確認済み環境以外での購入は **購入者の責任**です。動かなくても返金は受け付けず、その構成へのサポートも保証しません。
...
- **対象外:** **iOS 16 以前**（非対応）での購入、および **iOS 17 / iOS 18**（未検証）で動かなかった場合。パッケージが入っても CarPlay で動かない場合も含みます。自己責任での購入であり、製品の欠陥ではありません。
