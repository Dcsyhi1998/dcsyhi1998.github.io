...
**CarCast** は、**CarPlay 非対応**のアプリを **iOS 17** および **iOS 26** の **CarPlay ダッシュボード**に追加できる Tweak です。

**設定 → CarCast** から Safari や YouTube、設定アプリなどを選び、CarPlay に接続すれば、純正アプリと同じようにホーム画面から起動できます。

> **動作確認は iOS 17 と iOS 26 です。** パッケージの最低依存は **iOS 17.0** です。**iOS 16 以前は非対応**です。**iOS 18 は未検証**です（iOS 17 と同じ実装ですが保証はありません）。動かなくても **購入者の責任**であり、**返金対象外**です。購入前に **設定 → 一般 → 情報** で OS バージョンを確認してください。
...
A. **iOS 17 は動作確認済みです。** **iOS 18** は未検証です（iOS 17 と同じ実装です）。**保証はありません。** 未検証の OS で購入して動かなくても、それは **購入者の責任**であり、**返金対象外**です。
...
A. **iOS 16 以前**は非対応です。**iOS 18** は未検証です。それらの環境で動かない購入は、インストールできても **返金ポリシーの対象外**です。**iOS 17** と **iOS 26** は動作確認済みです。
...
- **購入前に必ず iOS バージョンを確認してください。** 作者が動作確認したのは **iOS 17** と **iOS 26** です。
- **iOS 16 以前**は **非対応**です。**iOS 18** は **未検証**です。確認済み環境以外での購入は **購入者の責任**です。動かなくても返金は受け付けず、その構成へのサポートも保証しません。
...
- **対象外:** **iOS 16 以前**（非対応）での購入、および **iOS 18**（未検証）で動かなかった場合。パッケージが入っても CarPlay で動かない場合も含みます。自己責任での購入であり、製品の欠陥ではありません。
