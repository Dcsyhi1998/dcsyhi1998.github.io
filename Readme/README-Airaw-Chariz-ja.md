...
- フルスクリーン用のオーバーレイ表示位置の設定
- アプリごとの表示スケールとノッチ設定
- コンテキストメニューとお気に入りアプリ設定
...
- 秒表示と言語・地域設定
- 上、下、左、右の表示位置の変更
- 左ハンドルまたは右ハンドル向けのレイアウトの変更
- 天気アイコンと気温表示
- バッテリー情報の表示(iPhoneやアップルウォッチや、Bluetoothで接続している)アイコンを表示最大3個まで
- ステータスバー各項目の表示色の変更(Wi-Fi,時間,ステータスバーの背景色,LET,DND と FOCUS,バッテリー,)
- ホームボタンのアイコンのカスタマイズ(自身の写真やiPhone独自のSF Symbols)で変更

### ウィジェット

対応するウィジェットをCarPlayのホーム画面に配置し、サイズや位置を変更できます。
...
- [CarScrub](https://dcsyhi1998.github.io/depiction/CarScrub.html): CarPlay の再生バーを、ドラッグできるようにします。
